...
void HelpDisplay::show_info(const std::string& message, std::ostream& output) {
    output << BLUE << "ℹ️  INFO: " << message << RESET << '\n';
}
...
void HelpDisplay::show_command_flags(std::ostream& output) {
    ...
    output << "   " << format_text("🔍 Scanning & Discovery", GREEN) << '\n';
    output << "      " << format_text("-scan", BLUE) << "                       " << format_text("<blockchain> [blocks]     Scan last blocks (default: 1000)", CYAN) << '\n';
    ...
    output << "   " << format_text("🏦 DEX Analysis", GREEN) << '\n';
    output << "      " << format_text("-showDEXES", BLUE) << "                   " << format_text("<blockchain>              Show all DEXes", CYAN) << '\n';
    output << "      " << format_text("-showPOOLS", BLUE) << "                   " << format_text("<blockchain> <DEX>         Show DEX pools", CYAN) << '\n';
    output << "      " << format_text("-showTOKENS", BLUE) << "                  " << format_text("<blockchain> <DEX>         Show DEX tokens", CYAN) << '\n\n';
