...
uint64_t get_pool_count(const std::string& rpc_url, const std::string& factory_address, int request_limit, FunctionStats& stats) {
    ...
    // Parse the result (Check if result is valid)
    if (!result.empty() && result != "0x0") {
        
        // Convert hex to integer and return
    ...
    // Return 0 if call failed or no pools
    return 0;
}

// Function to get the address of a pool
std::string get_pool_address(const std::string& rpc_url,
    ...
    // Stream to build data field
    std::stringstream data;
    
    // Build call data
    data << "0x1e83409a" << std::setfill('0') << std::setw(64) << std::hex << index;
    ...
    if (!result.empty() && result != "0x") {
        ...
        // Extract and return pool address
        return "0x" + result.substr(26);
    }
    ...
    // Return empty string if call failed
    return "";
}
