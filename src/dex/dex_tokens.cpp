...
std::pair<std::string, std::string> get_pool_tokens(const std::string& rpc_url, const std::string& pool_address, int /* request_limit */, FunctionStats& stats) {
    ...
    std::string result0 = make_rpc_call(rpc_url, payload0, 0, stats); // Make RPC call
    ...
    std::string result1 = make_rpc_call(rpc_url, payload1, 0, stats); // Make RPC call
